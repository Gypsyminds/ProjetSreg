...
@Repository
public interface CommentairePostsRepository extends CrudRepository<Commentaires_post,Long> {
   ...
   // @Query(value = "select score from question_qcm  score  where id_qs_qcm= :id_qs_qcm",nativeQuery = true)
    ...
    List<Commentaires_post> findRandom(@Param("certif_id_test") long certif_id_test);
